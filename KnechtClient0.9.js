...
(function ()
{
    ...
    function _sendRequest ( method, path, callback, body ) //sends a request to the server and passes result to callback
    {
        ...
        request.onreadystatechange = function ()
        {
            ...
            {
                if(_isJSON(request.responseText)) callback( request.status, JSON.parse(request.responseText ));
                else {callback(500,{error: 'Response Text Was Not JSON'});}
            }
        };
    ...
    {
        if(typeof(callback) === 'function') _error_callback = callback;
    };
    //endregion
    ...
    K.checkUserRegistered = function ( username, callback )
    {
        _sendRequest( "GET",
            ...
            function ( status, result )
            {
                ...
                callback( result );
                if(result['error'] && _error_callback) _error_callback('checkUserRegistered', result['error']);
            } );
    };
    /**
    ...
    K.register = function ( username, password, callback, timeout )
    {
        ...
        _sendRequest( "POST",
            ...
            function ( status, result )
            {
                ...
                callback(result);
                if(result['error'] && _error_callback) _error_callback('register', result['error']);
            }, _password );
    };
    /**
    ...
    K.logout = function(callback)
    {
        ...
        _sendRequest("DELETE", "users/session?session_id=" + _session_id,
            function(status, result)
            {
                result['status'] = K.responses[status];
                callback(result);
                if(result['error'] && _error_callback) _error_callback('logout', result['error']);
            });
    };
    //endregion
    ...
    K.recoverPassword = function(username, callback)
    {
        _sendRequest("GET",
            ...
            function(status, result)
            {
                result['status'] = K.responses[status];
                callback(result);
                if(result['error'] && _error_callback) _error_callback('recoverPassword', result['error']);
            });
    };
        ...
        _sendRequest("PUT",
            ...
            function ( status, result )
            {
                if ( K.responses[status] === K.UNAUTHORIZED )
                {
                    K.login( _username, _password, function ( login_result )
                    {
                        if ( login_result['status'] === K.OK ) K.changePassword(new_password, callback );
                        else
                        {
                            result['status'] = K.UNAUTHORIZED;
                            callback(result);
                ...
                if(result['error'] && _error_callback) _error_callback('changePassword', result['error']);
            }, new_password );
    };
    //endregion
    ...
    K.listGroupsOfApp = function(callback)
    {
            ...
            function(status, result)
        {
            result['status'] = K.responses[status];
            callback(result);
            if(result['error'] && _error_callback) _error_callback('listGroupsOfApp', result['error']);
        });
    }
    /**
    ...
    K.checkGroupPassword = function (group, password, callback)
    {
        ...
        function(status, result)
        {
            result['result'] = K.responses[status];
            callback( result );
            if(result['error'] && _error_callback) _error_callback('checkGroupPassword', result['error']);
        }, password);
    };
    //endregion
    ...
    K.listMembersOfGroup = function ( group, callback )
    {
            ...
            function ( status, result )
        {
            result['status'] = K.responses[status];
            callback(result);
            if(result['error'] && _error_callback) _error_callback('listMembersOfGroup', result['error']);
        } );
    };
    /**
    ...
    K.submitUpdates = function(group, fields, data, callback, permissions, members)
    {
        ...
        if(permissions) query_string += "&permissions=" + encodeURIComponent(JSON.stringify(permissions));
        if(members) query_string += "&members=" + encodeURIComponent(JSON.stringify(members));
      _sendRequest("PUT",
          query_string,
          function ( status, result )
          {
        ...
        _sendRequest( "GET",
            ...
            function ( status, result )
            {
                if ( K.responses[status] === K.UNAUTHORIZED )
                {
                    K.login( _username, _password, function ( login_result )
                    {
                        if ( login_result['status'] === K.OK ) K.listenUpdates( group, callback, timestamp );
                        else
                        {
                            result['status'] = K.UNAUTHORIZED;
                            callback(result);
                ...
                else
                {
                    result['status'] = K.responses[status];
                    callback(result);
                    if(result['updates'] !== undefined) K.listenUpdates(group, callback, result['timestamp']);
                }
                if(result['error'] && _error_callback) _error_callback('listenUpdates', result['error']);
            } );
    };
    //endregion
        ...
        _sendRequest( "GET",
            ...
            function ( status, result )
            {
                if ( K.responses[status] === K.UNAUTHORIZED )
                {
                    K.login( _username, _password, function ( login_result )
                    {
                        if ( login_result['status'] === K.OK ) K.listenInputs( group, callback, timestamp );
                        else
                        {
                            result['status'] = K.UNAUTHORIZED;
                            callback(result);
