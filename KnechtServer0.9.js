(function()
{
    ...
    var port = 8080; //Server port the server will be listening on
    var db_config =
    {
        host: 'localhost',
        user: 'root',
    ...
    function _connect() //recommended disconnect handler from https://github.com/felix/node-mysql/blob/master/Readme.md
    {
        ...
        {
            if(err) setTimeout(_connect, db_config.reconnect_delay); //if connecting fails, try again after delay
        });
    ...
    function _checkCredentials(session_id, response, callback) //invalid if wrong session_id, unauthorized if expired
    {
        connection.query("SELECT username, timeout, last_ping FROM users WHERE session_id = ? LIMIT 1;",
            ...
            function(err, result)
            {
                ...
                else if(result.length === 0) _finishResponse(INVALID, response, err_msg.invalid_session);
                else if(result[0]['last_ping'] + result[0].timeout * 60000 <= _getTime())
                {
                    _finishResponse(UNAUTHORIZED, response, err_msg.expired_session);
                }
                else connection.query("UPDATE users SET last_ping = ? WHERE username = ? LIMIT 1;",
    ...
    function _checkHost(username, group, response, callback) //invalid if group doesnt exist, unauthorized if not host
    {
        connection.query("SELECT host FROM groups WHERE name = ? LIMIT 1;",
            ...
            {
                if(err) _finishResponse(ERROR, response, err_msg.db_err);
                else if (result.length === 0) _finishResponse(INVALID, response, err_msg.no_group);
                else if(result[0].host !== username) _finishResponse(UNAUTHORIZED, response, err_msg.not_host);
                else callback(); //proceed to calling function
            });
    }
    //endregion
