...
(function ()
{
    ...
    //region Constants
    //mapping between http response codes and Knecht result strings
    K.responses = {
    ...
    //these functions are used by the other functions in this document; they are not invoked by the application

    //this function sends an Ajax request to the server
    //the method parameter is a string that is an http request method, defining the action to be taken on the resource
    //the path parameter is the URI of the server resource being requested
    ...
    //  an object that is the value returned from the server, containing at minimum a timestamp member
    ...
    //specifies the name of the application using Knecht. Must be called before any single-user data functions are called
    ...
    //registered, which is a boolean that is true if the user exists and false if they do not
    //this function returns nothing
    K.checkUserRegistered = function ( email, callback )
    {
        _sendRequest( "GET", "/users?email=" + encodeURIComponent( email ), function ( status, result )
        {
            result['result'] = K.responses[status];
            callback( result );
        } );
    };
    ...
    //this function returns nothing
    //if the response to this request indicates success, the session_id returned will be stored locally for further use
    ...
    K.login = function ( email, password, callback )
    {
        _email = encodeURIComponent( email );
        _password = password;
        _sendRequest( "PUT", "/users/session?email=" + _email, function ( status, result )
        {
            if ( K.responses[status] === K.OK )
            {
    ...
    K.logout = function(callback)
    {
        _email = null;
        _password = null;
        _sendRequest("DELETE", "users/session?session_id=" + _session_id, function(status, result)
        {
            result['status'] = K.responses[status];
            callback(result);
        });
    ...
    K.putData = function ( field, data, callback )
    {
        _sendRequest( "PUT", "/users/data?session_id=" + _session_id +
            "&app=" + _app + "&field=" + encodeURIComponent(JSON.stringify(field)), function ( status, result )
        {
            if ( K.responses[status] === K.UNAUTH )
            {
                K.login( _email, _password, function ( result )
    ...
    K.hostSubmit = function (group, fields, data, users, permissions, callback )
    {
        _sendRequest( "PUT", "/groups/data/permissions?session_id=" + _session_id +
            ...
            "&permissions=" + encodeURIComponent(JSON.stringify(permissions))), function ( status, result )
        {
            if ( K.responses[status] === K.UNAUTH )
            {
                K.login( _email, _password, function ( result )
                {
                    if ( result['status'] === K.OK )
                    {
                        K.hostSubmit( group, fields, data, users, permissions, callback );
                    }
                    else
                    {
                        result['status'] = K.UNAUTH;
    ...
    K.setPermissions = function ( group, fields, users, permissions, callback )
    {
        _sendRequest( "PUT", "/groups/data/permissions?session_id=" + _session_id +
            "&group=" + encodeURIComponent( group ) +
            "&fields=" + encodeURIComponent( JSON.stringify(fields) ) +
            ...
            "&permissions=" + encodeURIComponent(JSON.stringify(permissions))), function ( status, result )
        {
            if ( K.responses[status] === K.UNAUTH )
            {
                K.login( _email, _password, function ( result )
                {
                    if ( result['status'] === K.OK )
                    {
                        K.setPermissions( group, fields, users, permissions, callback );
                    }
                    else
                    {
                        result['status'] = K.UNAUTH;
